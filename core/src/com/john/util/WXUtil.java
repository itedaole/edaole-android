...
public class WXUtil {
	...
	private final static int THUMB_WDITH = 120;
	private final static int THUMB_HEIGHT = 120;
	public static byte[] bmpToByteArray(final Bitmap bmp,
			final boolean needRecycle) {

		Rect source = new Rect(0, 0, bmp.getWidth(), bmp.getHeight());
		Rect target = new Rect(0, 0, THUMB_WDITH, THUMB_HEIGHT);

		Bitmap thumb = Bitmap.createBitmap(120, 120, Config.RGB_565);
		Canvas canvas = new Canvas(thumb);
		canvas.drawColor(Color.TRANSPARENT);
		canvas.drawBitmap(bmp, source, target, null);

		ByteArrayOutputStream output = new ByteArrayOutputStream();
		thumb.compress(CompressFormat.PNG, 0, output);
	...
	public static byte[] httpGet(final String url) throws IOException,
